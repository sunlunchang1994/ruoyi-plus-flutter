#!/bin/bash
# 一键打包 Windows 桌面应用（包含完整的清理和构建流程）
...
# 步骤5: 打包 Windows
echo -e "\n${BLUE}[5/5] 打包 Windows 桌面应用...${NC}"
cd "$PROJECT_ROOT"
flutter build windows --release
if [ $? -ne 0 ]; then
    echo -e "${RED}打包 Windows 失败！${NC}"
    exit 1
fi
echo -e "${GREEN}✓ Windows 打包完成${NC}"

# 显示Windows输出路径
WINDOWS_PATH="$PROJECT_ROOT/build/windows/x64/runner/Release"
if [ -d "$WINDOWS_PATH" ]; then
    echo -e "\n${GREEN}========================================"
    echo -e "打包成功！"
    echo -e "========================================${NC}"
    echo -e "${YELLOW}Windows 应用路径：${NC}"
    echo -e "$WINDOWS_PATH"
    
    # 获取目录大小
    if [[ "$OSTYPE" == "darwin"* ]]; then
        SIZE=$(du -sh "$WINDOWS_PATH" | cut -f1)
    else
        SIZE=$(du -sh "$WINDOWS_PATH" | cut -f1)
    fi
    echo -e "${YELLOW}总大小：${NC}$SIZE"
    echo -e "\n${BLUE}提示：可以将整个 Release 文件夹打包分发${NC}"
else
    echo -e "${RED}警告: Windows 输出目录未找到！${NC}"
fi
