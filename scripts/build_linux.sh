...
# 步骤5: 打包 Linux
echo -e "\n${BLUE}[5/5] 打包 Linux 桌面应用...${NC}"
cd "$PROJECT_ROOT"
flutter build linux --release
if [ $? -ne 0 ]; then
    echo -e "${RED}打包 Linux 失败！${NC}"
    exit 1
fi
echo -e "${GREEN}✓ Linux 打包完成${NC}"

# 显示Linux输出路径
LINUX_PATH="$PROJECT_ROOT/build/linux/x64/release/bundle"
if [ -d "$LINUX_PATH" ]; then
    echo -e "\n${GREEN}========================================"
    echo -e "打包成功！"
    echo -e "========================================${NC}"
    echo -e "${YELLOW}Linux 应用路径：${NC}"
    echo -e "$LINUX_PATH"
    
    # 获取目录大小
    SIZE=$(du -sh "$LINUX_PATH" | cut -f1)
    echo -e "${YELLOW}总大小：${NC}$SIZE"
    
    echo -e "\n${BLUE}提示：可以将整个 bundle 文件夹打包分发${NC}"
else
    echo -e "${RED}警告: Linux 输出目录未找到！${NC}"
fi
