...
abstract class SysNoticeApiClient {
  factory SysNoticeApiClient({Dio? dio, String? baseUrl}) {
    ...
    return _SysNoticeApiClient(dio,
  ...
  Future<ResultEntity> add(
      @Body() SysNotice? data, @CancelRequest() CancelToken cancelToken);
...
class SysNoticeRepository {
    ...
    return _sysConfigApiClient
      ...
      return event.toIntensify(
          createData: (resultEntity) => resultEntity.toPageModel(offset, size,
                ...
                List<SysNotice> sysNoticeList =
                    SysNotice.fromJsonList(resultData);
  ...
  ///获取通知公告信息
  static Future<IntensifyEntity<SysNotice?>> getInfo(
  ...
  ///提交通知公告信息
  static Future<IntensifyEntity<SysNotice>> submit(
      SysNotice body, CancelToken cancelToken) {
    ...
    if (sysNoticeList?.isNotEmpty ?? false) {
      for (var itemData in sysNoticeList!) {
        itemData.noticeTypeName = GlobalVm()
            .dictShareVm
            .findDict(
                LocalDictLib.CODE_SYS_NOTICE_TYPE, itemData.noticeType)
            ?.tdDictLabel;
      }
    }
  }
