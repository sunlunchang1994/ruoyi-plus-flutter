...
class BaseDio {
  ...
  Dio getDio() {
    ...
    dio.options = BaseOptions(
        receiveTimeout: const Duration(seconds: 15000), connectTimeout: const Duration(seconds: 15000)); // 设置超时时间等 ...
  ...
  ///获取错误码
  int getErrorCode(Exception exception, {String? defErrMsg}) {
    return getError(exception, defErrMsg: defErrMsg).code!;
  }
  ...
  ResultEntity getError(Exception exception, {String? defErrMsg}) {
    ...
    switch (exception.runtimeType) {
      case DioException _:
        if ((exception as DioException).type == DioExceptionType.badResponse) {
